...
**A second opinion, on request.** `--hep-send host:port` forwards a copy of every SIP message the sensor observes, encapsulated as HEP v3 (the Homer/EEP capture protocol, generic chunks only), to an external UDP collector — so whoever runs one can hold every condemnation against the packets that caused it, without this project growing a packet store, a message database or a UI. Off unless asked for: absent, no socket is opened and nothing runs on the packet path. Present, the copy leaves through a bounded queue on its own thread, so a collector that cannot keep up costs drops — counted, printed at every stats interval as `HEP to <collector>: hep_sent= hep_dropped= hep_failed=`, and carried on the checkpoint line that `tfps_ctl stats` shows — never capture (§10). `--hep-agent-id N` names this sensor in the stream. Plain HEP asserts its own source and destination, so a collector that acts on the copy — a kill response, a jail — could be aimed by anyone who can reach its port; `--hep-auth-file PATH` gives the sensor the collector's shared secret, refused if the world can read the file, and `--hep-auth-mode` chooses how it travels: `hmac`, the default, signs every packet with a per-message token (a timestamp, a nonce and an HMAC-SHA256 over the whole datagram, addressing chunks included) that the collector verifies and a captured copy cannot replay; `plain` sends the key verbatim, which is what a stock Homer understands and what anyone on the path can read.

**Evidence in, on request.** The reverse channel is `tfps_ctl ingest`: whoever runs a collector may hand findings back — one JSON object per line, `{"src_ip","rule","evidence"}` — and this system decides what to do with them. Nothing outside it ever bans anything. A finding is applied exactly as an operator's `tfps_ctl ban` is applied: the host's own addresses and `ignoreip` are refused, the TTL is the operator's, and the audit row carries `sipnab:<rule>` with the evidence as its detail, so the label export (§12) says who asked. A line that is not a finding is reported as `invalid` on its own line and never stops the stream. `--dry-run` reports every decision and writes nothing.
